...
CREATE VIEW vw_MarvelSuperheroes AS
SELECT 
...
INSERT INTO superhero (id, full_name, superhero_name, publisher_id)
VALUES (101, 'Peter Parker', 'Spider-Man', (SELECT id FROM publisher WHERE publisher_name LIKE '%Marvel%'));



-- Create a view to manage deletion of DC superheroes
CREATE VIEW vw_DCSuperheroes AS
SELECT 
    s.id AS superhero_id,   
    s.full_name,
    s.superhero_name,
...
WHERE superhero_id = (
    ...
    WHERE p.publisher_name LIKE '%DC%'
);
...
-- Use the existing view for DC superheroes (created earlier)
GRANT DELETE ON vw_DCSuperheroes TO DCComicsManager;
...
-- Create a role for managing superhero attributes
CREATE ROLE SuperheroAttributeManager;

-- Grant see all the superheros 
GRANT SELECT ON superhero TO SuperheroAttributeManager;
