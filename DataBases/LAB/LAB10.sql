...
(1, 'John', 'Abraham', 1000000, '01-JAN-16 12.00.00 AM', 'Banking'),
(2, 'Michael', 'Clarke', 800000, '01-JAN-16 12.00.00 AM', 'Insurance'),
(3, 'Roy', 'Thomas', 700000, '01-JAN-16 12.00.00 AM', 'Banking'),
(4, 'Tom', 'Jose', 600000, '01-JAN-16 12.00.00 AM', 'Insurance'),
(5, 'Jerry', 'Pinto', 650000, '01-JAN-16 12.00.00 AM', 'Insurance'),
(6, 'Philip', 'Mathew', 750000, '01-JAN-16 12.00.00 AM', 'Services'),
(7, 'TestName1', '123', 650000, '01-JAN-16 12.00.00 AM', 'Services'),
(8, 'TestName2', 'Lname%', 600000, '01-JAN-16 12.00.00 AM', 'Insurance');
...
SELECT FIRST_NAME, YEAR(CONVERT(DATETIME, JOINING_DATE, 120)) AS Joining_Year,
       MONTH(CONVERT(DATETIME, JOINING_DATE, 120)) AS Joining_Month,
       DAY(CONVERT(DATETIME, JOINING_DATE, 120)) AS Joining_Day
FROM Employees;
...
SELECT * FROM Employees WHERE FIRST_NAME LIKE '%n%' AND FIRST_NAME LIKE '%[A-Za-z]%';

-- Task 25: Get employee details from employee table whose first name starts with 'J' and contains letters
SELECT * FROM Employees WHERE FIRST_NAME LIKE 'J%' AND FIRST_NAME LIKE '%[A-Za-z]%';
...
SELECT * FROM Employees WHERE YEAR(CONVERT(DATETIME, JOINING_DATE, 120)) = 2016;

-- Task 31: Get employee details from employee table whose joining month is “January”
SELECT * FROM Employees WHERE MONTH(CONVERT(DATETIME, JOINING_DATE, 120)) = 1;

-- Task 32: Get employee details from employee table who joined before January 1st, 2016
SELECT * FROM Employees WHERE CONVERT(DATETIME, JOINING_DATE, 120) < '2016-01-01';

-- Task 33: Get employee details from employee table who joined after January 31st, 2016
SELECT * FROM Employees WHERE CONVERT(DATETIME, JOINING_DATE, 120) > '2016-01-31';

-- Task 34: Get Joining Date and Time from employee table
SELECT CONVERT(DATETIME, JOINING_DATE, 120) AS Joining_DateTime FROM Employees;

-- Task 35: Get Joining Date, Time including milliseconds from employee table
SELECT FORMAT(CONVERT(DATETIME, JOINING_DATE, 120), 'yyyy-MM-dd HH:mm:ss.fff') AS Joining_DateTime_Milli FROM Employees;
...
SELECT e.* 
FROM Employees e
WHERE EXISTS (SELECT 1 FROM Incentives i WHERE e.EMPLOYEE_ID = i.Employee_Ref_ID);

-- Task 60: How to fetch data that are common in two query results?
-- Use INTERSECT to fetch common data.
SELECT FIRST_NAME FROM Employees
INTERSECT
SELECT FIRST_NAME FROM Incentives;
...
WHERE Employee_Ref_ID = (SELECT EMPLOYEE_ID FROM Employees WHERE FIRST_NAME = 'John');
